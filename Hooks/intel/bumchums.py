...
class bumchums(loadable):
    ...
    def execute(self, message, user, params):
        ...
        Q = session.query(M.DB.Maps.Planet, M.DB.Maps.Intel, M.DB.Maps.Alliance, count())
        ...
        Q = Q.filter(M.DB.Maps.Alliance.id==alliance.id)
        ...
        if len(result) < 1:
            ...
            return
        prev=[]
        for planet, intel, alliance, count in result:
            prev.append("%s:%s (%s)"%(planet.x, planet.y, count))
