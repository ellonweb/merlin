{% extends "planets.tpl" %}
{% block title %}<a href="{% url "alliance" alliance.name %}">{{ alliance.name }}</a> (<a href="{% url "alliance_history", alliance.name %}">History</a>), list of {{ planets|count }} members in intel ({{ alliance.members }} in tag){% endblock %}
