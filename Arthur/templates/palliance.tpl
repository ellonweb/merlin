{% extends "planets.tpl" %}
{% block title %}<a href="{% url "alliance" alliance.name %}">{{ alliance.name }}</a> (<a href="{% url "alliance_history", alliance.name %}">History</a>), list of {{ members }} members in intel ({{ alliance.members }} in tag){% endblock %}
...
{% block sort_rank %}<a href="{% url "alliance_planets", alliance.name, race|default("all"), order|lower, page|default(1) %}">{{ order }}</a>{% endblock %}
